...
multiple events per time point are possible.
...
one event per subject by the maximum grade experienced. To count the maximum grade 
...
In this case, the count reported in any cell cannot exceed the number of subjects.
...
Note that the input `by` variable can be numeric (e.g., grade 1, 2, 3) or factor
(e.g. attribution unlikely, possible, definite). In the respective cases,
the maximum value observed would be according to numeric order or factor
level order.
